...
from pipeline.tokenizer import FullTokenizer, tokenize, remove_stopwords

def load_data(lst):
    reviews, scores = [], []

    for i in lst:
        score, review = i.split(' ', 1)
...
class rt:
    ...
    def process(self, x, wv):
        x = map(tokenize, x)
        x = map(remove_stopwords, x)
        x = wv.preprocess(x)
        return x

    def simple_data(self):
        wv = LM.pretrained_2(max_len=self.seq_len, padding_item=[300*[0]])
